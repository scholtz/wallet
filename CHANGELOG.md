...
- Added a Simulation section to every "transactions to sign" screen (WalletConnect requests and multi-signature signing): before you sign, the wallet dry-runs the exact transaction group against the current network state and shows every application state change it would cause — which smart contract, whether it's a global or per-account value, the key, and whether it's being set or deleted — including changes made by inner transactions. If the group would fail, the failure reason is shown up front instead of only after signing and submitting.
