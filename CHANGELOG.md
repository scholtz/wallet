...
- Added an "Explore routes" button to the Swap page: after getting a quote, you can now open a visual breakdown of exactly which pools and protocols each enabled DEX aggregator (Haystack, Folks Router, Biatec Router) plans to route your swap through, including the amount moving in and out of each hop, price impact, and network fees — with one-click copy buttons on every figure and on the raw quote data.
